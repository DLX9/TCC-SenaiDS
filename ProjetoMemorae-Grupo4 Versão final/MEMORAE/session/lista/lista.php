...
<body class="back">
  ...
  <header class="head">
    ...
    <a href="lista.php"><button type="button" class="btn btn-success fw-bold"> Consultas </button></a>
    ...
    $CON = "SELECT id, id_usuario, id_clinica, id_medico, dia, horario, ativo FROM consulta WHERE ativo = 1 AND id_usuario = $id";
    $consultacon = mysqli_query($connect, $CON);

    //Estrutura de repetição para cada consulta
    if(mysqli_num_rows($consultacon) > 0){
      while($consu = mysqli_fetch_assoc($consultacon)){
        $idm = $consu['id_medico'];
        $idc = $consu['id_clinica'];

        $CONM = "SELECT nome FROM medico WHERE ativo = 1 AND id = $idm";
        $CONC = "SELECT nome FROM clinica WHERE ativo = 1 AND id = $idc";

        $consultam = mysqli_query($connect, $CONM);
        $consultac = mysqli_query($connect, $CONC);

        $dmed = mysqli_fetch_assoc($consultam);
        $dcli = mysqli_fetch_assoc($consultac);
      ...
      <div class="repc fw-bold">
        <div>
          <div class="ctil">
            <p> <?php print($nmed); ?> - <?php print($ncli); ?> </p>
          </div>
          <div class="cdate">
            <p> Dia: <?php print($consu["dia"]); ?> - Hora: <?php print($consu["horario"]); ?> </p>
          </div>
        </div>
          <form action="proccess-consulta.php" method="post" class="d-flex justify-content-around">
            <input type="text" value="<?php print($consu['id']) ?>" id="idcon" name="idcon" hidden>
            <button name="alterar" value="reagendar" type="submit" class="btn btn-secondary fw-bold text-light"> Reagendar </button>
