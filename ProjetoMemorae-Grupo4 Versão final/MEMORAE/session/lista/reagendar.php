...
<body class="back">
    ...
    <?php
        ...
        if(isset($idcon)){
            ...
            $DCON = "SELECT id_clinica, id_medico FROM consulta WHERE id = $idcon";
            $consultacon = mysqli_query($connect, $DCON);
            $DDCON = mysqli_fetch_assoc($consultacon);

            $DCL = "SELECT nome FROM clinica WHERE id = $DDCON[id_clinica]";
            $consultacl = mysqli_query($connect, $DCL);
            $NCL = mysqli_fetch_array($consultacl);

            $DMD = "SELECT nome FROM medico WHERE id = $DDCON[id_medico]";
    ...
    <center>
    <h2 class="text-uppercase top">Reagendando...</h2>
    <form action="proccess-reagendar.php" method="post" class="main">
        ...
        <input type="text" name="clinica" id="clinica" class="col-5" value="<?php print($DDCON["id_clinica"]); ?>" hidden>
        <input type="number" name="consulta" id="consulta" class="col-5" value="<?php print($idcon); ?>" hidden>
        <br>
        ...
        <!--Input hidden que carrega o valor da variavel-->
        <input type="text" name="medico" id="medico" class="col-5" value="<?php print($DDCON["id_clinica"]); ?>" hidden>
        <br>
