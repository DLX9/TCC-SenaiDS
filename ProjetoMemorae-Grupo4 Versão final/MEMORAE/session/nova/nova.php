...
<body class="back">
    ...
    <form action="medico.php" method="post" class="main">

        <label for="clinica" class="col-1">Clinica:</label>
        <select name="clinica" id="clinica" class="col-5">
        <option value="">Escolha uma clinica</option>
        
        <?php 
        //Puxa do banco de dados as clinicas cadastradas
        $DCL = "SELECT id, nome FROM clinica WHERE ativo = 1;";
        $consultac = mysqli_query($connect, $DCL);
        if(mysqli_num_rows($consultac) > 0){
        while($dadosc = mysqli_fetch_assoc($consultac)){
            ?>
            <option value="<?php print($dadosc["id"]);?>"> <?php print($dadosc["nome"]);?> </option>
            <?php
        };
        }
        ...
        <a href="medico.php"><button type="submit" class="btn btn-success fw-bold">Proximo Passo</button></a>
    </form>
    <br>
