...
<body class="back">
    ...
    <form action="quando.php" method="post" class="main">

        <?php
        //Recebe informações de outra página via metodo post
        $clinica = $_POST["clinica"];
        //Variavel que puxa dados do BD
        $DCL = "SELECT nome FROM clinica WHERE ativo = 1 AND id = $clinica";
        $consultac = mysqli_query($connect, $DCL);
        $NCL = mysqli_fetch_array($consultac);
        ?>
          ...
          <label for="medico" class="col-1">Doutor:</label>
          <select name="medico" id="medico" class="col-5">
          <option value="">Escolha um doutor</option>

          <?php
          $DMD = "SELECT id, nome FROM medico WHERE ativo = 1 AND id_clinica = $clinica";
          $consultam = mysqli_query($connect, $DMD);
          if(mysqli_num_rows($consultam) > 0){
          while($medico = mysqli_fetch_assoc($consultam)){
            ?>
            <option value="<?php print($medico["id"]);?>"> <?php print($medico["nome"]);?> </option>
            <?php } } 
          ...
          <br>
          <a href="quando.php"><button type="submit" class="btn btn-success fw-bold">Proximo Passo</button></a>
          </form>
          <br>
